...
def process_image(image_file, mode):
    """
    ...
    try:
        ...
        if mode == 'ai_enhancer':
            result = enhance_image(image_array, scale=2)
        ...
        logger.info(f"Image processing completed successfully for mode: {mode}")
        return result_image
...
def image_to_base64(image):
    ...
    try:
        ...
        # Use PNG for grayscale to preserve quality, JPEG for color
        if image.mode == 'L':
